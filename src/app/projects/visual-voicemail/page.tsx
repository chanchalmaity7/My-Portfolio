...
export default function VisualVoicemailProjectPage() {
  return (
    <main className="min-h-screen bg-[#07111f] text-white">
      <section className="relative overflow-hidden border-b border-white/10 bg-[radial-gradient(circle_at_18%_20%,rgba(37,99,235,0.22),transparent_30%),radial-gradient(circle_at_82%_10%,rgba(16,185,129,0.18),transparent_28%),linear-gradient(135deg,#07111f_0%,#0d1727_52%,#08111f_100%)] px-6 pb-20 pt-28">
        <div className="container mx-auto grid items-center gap-12 lg:grid-cols-[1.08fr_0.92fr]">
          <div className="max-w-4xl">
            ...
            <p className="mt-10 text-sm font-semibold uppercase tracking-[0.26em] text-cyan-200">Telephony workflow case study</p>
            <h1 className="mt-4 text-5xl font-black tracking-tight sm:text-6xl lg:text-7xl">{voicemailProjectCaseStudy.title}</h1>
            <p className="mt-3 text-lg font-semibold text-emerald-200">{voicemailProjectCaseStudy.subtitle}</p>
            <p className="mt-8 max-w-4xl text-lg leading-9 text-slate-200 sm:text-xl">{voicemailProjectCaseStudy.longDescription}</p>
            ...
            <div className="mt-6 rounded-[1.75rem] border border-emerald-300/20 bg-emerald-300/10 p-5 text-sm leading-8 text-emerald-100 sm:text-base">
              {voicemailProjectCaseStudy.soloNarrative}
            </div>
          ...
          <div className="relative">
            ...
            <div className="relative overflow-hidden rounded-[2rem] border border-white/10 bg-white/[0.06] p-6 shadow-2xl shadow-slate-950/40 backdrop-blur-xl">
              <div className="relative aspect-[4/3] w-full overflow-hidden rounded-[1.7rem] border border-white/10 bg-slate-950/80">
                <Image src={voicemailProjectCaseStudy.assets.poster} alt="Visual Voicemail System poster" fill sizes="(min-width: 1024px) 38vw, 100vw" className="object-cover" priority />
              </div>
            </div>
          </div>
        </div>
      ...
      <section className="border-t border-white/10 bg-[#0b1220] px-6 py-14">
        <div className="container mx-auto grid gap-4 md:grid-cols-2 xl:grid-cols-3">
          {voicemailProjectCaseStudy.heroMetrics.map((metric) => (
      ...
      <section className="bg-[#07111f] px-6 py-18">
        <div className="container mx-auto">
          <div className="max-w-4xl">
            ...
            <h2 className="mt-4 text-4xl font-black tracking-tight sm:text-5xl">What makes this product deeper than a simple call log</h2>
          </div>
          <div className="mt-10 grid gap-4 md:grid-cols-2">
            {voicemailProjectCaseStudy.homeProofBlocks.map((item) => (
      ...
      <section className="bg-[#0b1220] px-6 py-18">
        <div className="container mx-auto grid gap-4 md:grid-cols-2 xl:grid-cols-3">
          {voicemailProjectCaseStudy.keySystems.map((system) => (
      ...
      <section className="bg-[#07111f] px-6 py-18">
        <div className="container mx-auto grid gap-4 md:grid-cols-2">
          {voicemailProjectCaseStudy.technicalArchitecture.map((group) => (
        ...
        <div className="container mx-auto grid gap-10 lg:grid-cols-[1.05fr_0.95fr]">
          <div>
            ...
            <div className="mt-8 grid gap-3">
              {voicemailProjectCaseStudy.soloOwnership.map((item) => (
          ...
          <div>
            ...
            <div className="mt-8 flex flex-wrap gap-3">
              {voicemailProjectCaseStudy.technologyList.map((tech) => (
